...
int main() {
	...
	x.InputName("Tom");
	Diary token;
	cout << "Tom,How many diaries do you want to input?\n";
	cin >> i;
	for (int n = 0; n < i; n++) {
	...
	DiaryBook z, w;
	z = x; w = y;
	cout << "The First diary book:\n"; x.output();
	cout << "The Second diary book:\n"; y.output();
	cout << "The Third diary book:\n"; z.output();
	cout << "The Fourth diary book:\n"; w.output();
	if (z == w)
...
void DiaryBook::output() {
	...
	for (int i = 0; i<Diaries.size(); i++) {
		cout << "The date of diary #" << i << " is " << Diaries[i].y << "." << Diaries[i].m << "." << Diaries[i].d << endl;
...
bool operator ==(const DiaryBook& d1, const DiaryBook& d2) {
	...
	for (int i = 0; i < d1.Diaries.size(); i++) {
		if (!strcmp(d1.Name, d2.Name) && d1.Diaries[i].y == d2.Diaries[i].y&&d1.Diaries[i].m == d2.Diaries[i].m&&d1.Diaries[i].d == d2.Diaries[i].d&&d1.Diaries[i].data == d2.Diaries[i].data)
			a = true;
	}
	return a;
}
